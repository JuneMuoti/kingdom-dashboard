...
function barColor(p: number) {
  if (p >= 100) return '#6ab0e8';
  if (p >= 60) return '#c4a050';
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 15, color: 'var(--text-primary)', marginBottom: 4, lineHeight: 1.3 }}>{p.title}</p>
          <p style={{ fontSize: 12, color: 'var(--text-secondary)' }}>👥 {p.team}</p>
        </div>
        <span style={{
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 500, color: barColor(p.progress) }}>{p.progress}%</span>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
        ...
        <span style={{ fontSize: 11, color: 'var(--gold)', background: 'var(--gold-dim)', padding: '2px 8px', borderRadius: 'var(--radius-pill)', border: '1px solid var(--gold-border)' }}>
          ⛰ {p.mountain}
        </span>
      </div>
